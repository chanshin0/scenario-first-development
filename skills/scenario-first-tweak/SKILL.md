...
- 판정 기준 = `design/design.md`(톤·원칙) + `design/tokens.md`(값). "예쁜가?"(직감) 가 아니라 "톤·값에 맞나?"(written rubric). 둘 다 없으면 직감 fallback (일관성 표류 위험 — 권장 안 함).
- `design/tokens.md` 가 있으면 다음을 **soft warn 으로** 본다 (import 그래프 머신 판정 없어 hard block 불가; SKILL §5 표면-커버리지 전례와 동일 톤. 게이트 신설 아님 — 게이트2 rubric 확장):
  - 생성물(`tokens.css`/`theme.css`)이 없거나, 진입 소스가 그걸 `@import` 안 함 → "토큰 안 물림 (later-add/swap retrofit 필요)".
  - tokens.md 를 고쳤는데 생성물이 옛 값(stale) → "재생성 필요".
  - 코드가 매직넘버(`#0d6efd` 직박) 씀 → "토큰 변수(`var(--accent)`)로".
- 스크린샷 / 전후 비교 / 실제 화면 확인.
- evidence 를 `scenarios/tweaks/<날짜>-<slug>.md` 에 기록 (`.harness/templates/REVIEW.md` 의 evidence 슬롯 양식 차용 — 최소 1개 시각 evidence + 어느 톤·토큰을 충족했는지 인용).
