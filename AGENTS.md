...
- `design/` (`design.md` 말 + `tokens.md` 값 SoT) — 프로젝트 외관 자산. behavior 직교 외관 축, **게이트 아님**(누적 풀 미진입). goal 이 tokens.md 를 스택 맞는 코드(:root/@theme)로 생성·import 하고 tweak 게이트2가 design.md(톤)+tokens.md(값)를 rubric 으로 인용. 톤·값 편집은 프로젝트 작업(sfd-architect 통로 밖), goal/tweak 의 참조·생성 메커니즘 변경만 하네스. (생성물 tokens.css/theme.css 는 파생 — 직접 편집 금지)
